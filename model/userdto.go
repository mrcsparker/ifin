...
type UserDTO struct {
	...
	UserGroups     []TenantEntity              `json:"userGroups"`     // The groups to which the user belongs. This field is read only and it provided for convenience. *Read Only*
	AccessPolicies []AccessPolicySummaryEntity `json:"accessPolicies"` // The access policies this user belongs to. *Read Only*
}
