...
type RemoteProcessGroupStatusSnapshotDTO struct {
	...
	ActiveThreadCount  int    `json:"activeThreadCount"`  // The number of active threads for the remote process group.
	FlowFilesSent      int    `json:"flowFilesSent"`      // The number of FlowFiles sent to the remote process group in the last 5 minutes.
	BytesSent          int    `json:"bytesSent"`          // The size of the FlowFiles sent to the remote process group in the last 5 minutes.
	Sent               string `json:"sent"`               // The count/size of the flowfiles sent to the remote process group in the last 5 minutes.
	FlowFilesReceived  int    `json:"flowFilesReceived"`  // The number of FlowFiles received from the remote process group in the last 5 minutes.
	BytesReceived      int    `json:"bytesReceived"`      // The size of the FlowFiles received from the remote process group in the last 5 minutes.
