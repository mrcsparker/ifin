...
type Processor struct {
	...
	State                      string          `json:"state"`                      // The state of the processor Allowable values: RUNNING, STOPPED, DISABLED
	Style                      interface{}     `json:"style"`                      // Styles for the processor (background-color => #eee).
	Relationships              []Relationship  `json:"relationships"`              // The available relationships that the processor currently supports.
	...
	Config                     ProcessorConfig `json:"config"`                     // The configuration details for the processor. These details will be included in a resopnse if the verbose flag is included in a request.
