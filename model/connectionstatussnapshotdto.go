...
type ConnectionStatusSnapshotDTO struct {
	...
	FlowFilesIn     int32  `json:"flowFilesIn"`     // The number of FlowFiles that have come into the connection in the last 5 minutes.
	BytesIn         int64  `json:"bytesIn"`         // The size of the FlowFiles that have come into the connection in the last 5 minutes.
	Input           string `json:"input"`           // The input count/size for the connection in the last 5 minutes, pretty printed.
	FlowFilesOut    int32  `json:"flowFilesOut"`    // The number of FlowFiles that have left the connection in the last 5 minutes.
	BytesOut        int64  `json:"bytesOut"`        // The number of bytes that have left the connection in the last 5 minutes.
	Output          string `json:"output"`          // The output count/sie for the connection in the last 5 minutes, pretty printed.
	FlowFilesQueued int32  `json:"flowFilesQueued"` // The number of FlowFiles that are currently queued in the connection.
	BytesQueued     int64  `json:"bytesQueued"`     // The size of the FlowFiles that are currently queued in the connection.
