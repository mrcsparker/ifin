...
UserAccessControl [] `json:"userAccessControl"` // The users that are allowed to access the port.
GroupAccessControl [] `json:"groupAccessControl"` // The user groups that are allowed to access the port.
ValidationErrors [] `json:"validationErrors"` // Gets the validation errors from this port. These validation errors represent the problems with the port that must be resolved before it can be started.
}
